...
import { EnrollmentSubmission, IdentifiedStudentView } from "@/server/types";
...
export async function identifyStudentForCampaign(
  ...
  if (!student) {
    ...
    throw new Error("Aluno não encontrado para o identificador fornecido.");
  }
  ...
  return {
    ...
    student: {
      ...
      maskedCc: maskSensitiveValue(student.cc),
      maskedNif: maskSensitiveValue(student.nif),
    },
    options: getEligibleClubOptionsForStudent(store, campaign, student),
  };
}
  ...
  return withStore((store) => {
    ...
    for (const choice of input.choices) {
      ...
      if (!clubOption || clubOption.remainingCapacity <= 0) {
        ...
        throw new Error("Uma das vagas deixou de estar disponível. Atualize a página e tente novamente.");
      }
    }
